...
// Helper to parse CSV properly (handling quotes, escaped quotes, and newlines)
...
export async function handleProductImport(event) {
    ...
    reader.onload = async (e) => {
        try {
            ...
            // Filter empty rows (e.g. trailing newlines)
            ...
            // Map data rows to objects using the header
            ...
            for (const [index, rowData] of productsData.entries()) {
                try {
                    ...
                    // Handle discount if column exists, otherwise default
                    ...
                    // Handle image if column exists (though fetching is disabled, we could store URL string if needed, but current logic expected base64)
                    // Since we removed image fetching, we just set null for new products.
