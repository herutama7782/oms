(function (global, factory) {
  ...
  // Kandidat service BLE printer yang umum
  ...
  class BLEPrinter {
      ...
      for (const svc of PRINTER_SERVICE_CANDIDATES) {
        try {
          ...
          if (writable) {
            ...
            // SOLUSI 4: Prioritaskan writeWithResponse jika tersedia.
            // Flag ini akan false jika 'write' ada, sehingga writeValueWithResponse akan digunakan.
    ...
    async write(data) {
      ...
      // Profil aman untuk HP (tanpa respons): chunk lebih kecil + delay lebih besar
      let CHUNK_SIZE = withResponse ? 180 : 18;
      let DELAY = withResponse ? 0 : 35;
      let DRAIN_EVERY_BYTES = withResponse ? 4096 : 2048;
      let DRAIN_DELAY = withResponse ? 50 : 300;

      // Penyesuaian tambahan untuk Android (lebih konservatif)
      const ua = (navigator.userAgent || '').toLowerCase();
      const isAndroid = ua.includes('android');
      if (!withResponse && isAndroid) {
        // SOLUSI 2: Kurangi ukuran chunk menjadi lebih kecil untuk Android
        CHUNK_SIZE = 10; // Dari 16 menjadi 10
        // SOLUSI 1: Naikkan delay secara signifikan untuk Android
        DELAY = Math.max(DELAY, 100); // Dari 40 menjadi 100ms
        DRAIN_DELAY = Math.max(DRAIN_DELAY, 500); // Naikkan juga delay drain
      }
      ...
      for (let i = 0; i < data.length; i += CHUNK_SIZE) {
        ...
        while (true) {
          try {
            ...
            attempt++;
            console.warn('Chunk write failed, backing off...', { attempt, error });

            // Degradasi adaptif saat gagal: perkecil chunk, tambah delay
            if (attempt <= 3) {
              await sleep(120);
              CHUNK_SIZE = Math.max(8, CHUNK_SIZE - 2); // Minimum 8
              DELAY = DELAY + 20;
              DRAIN_DELAY = DRAIN_DELAY + 50;
              chunk = data.slice(i, Math.min(i + CHUNK_SIZE, data.length));
              continue; // coba ulang chunk yang sama
            }
            throw error;
          }
        }
  ...
  class Printer {
    ...
    async printQRCode(text, options = {}) {
      ...
      const setup = [
        ...
        0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x43, sizeMap[size] || 6,
        0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x45, ecMap[errorCorrection] || 49
      ];
    ...
    async printBarcode(text, options = {}) {
      ...
      const cmds = [
        0x1B, 0x61, (align === 'center' ? 1 : align === 'right' ? 2 : 0),
        0x1D, 0x68, Math.max(1, Math.min(255, height)),
        0x1D, 0x77, Math.max(1, Math.min(6, width)),
    ...
    // Cetak gambar dipecah per stripe agar aman di HP
    async printImage(imageUrl, options = {}) {
      const defaultMaxWidth = this.paperWidth === 48 ? 576 : 384; // 80mm:576, 58mm:384
      // SOLUSI 3: Kurangi tinggi stripe gambar untuk mengurangi beban per pengiriman
      const { align = 'center', maxWidth = defaultMaxWidth, threshold = 128, stripeHeight = 32 } = options; // Dari 64 menjadi 32
      ...
      try {
        ...
        // Kirim per stripe (GS v 0) agar buffer tidak penuh
        for (let y0 = 0; y0 < heightInPixels; y0 += stripeHeight) {
          const h = Math.min(stripeHeight, heightInPixels - y0);

          // Header raster image untuk stripe ini
          const cmd = [0x1D, 0x76, 0x30, 0];
          cmd.push(widthInBytes & 0xFF, (widthInBytes >> 8) & 0xFF);
          cmd.push(h & 0xFF, (h >> 8) & 0xFF);
          ...
          // jeda kecil antar-stripe untuk HP
          await sleep(40);
        }

        // Reset alignment dan feed kecil untuk flush
    ...
    // Alias biar sesuai dokumentasi PrintHub
    async putImageWithUrl(url, options = {}) {
      return this.printImage(url, options);
    }
  }
