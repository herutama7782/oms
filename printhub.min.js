(function (global, factory) {
    ...
    class BLEPrinter {
        ...
        async write(data) {
            ...
            // --- START OF FIX ---
            // Manually convert the command string to a Uint8Array by character code.
            // This is crucial because ESC/POS commands are raw byte sequences,
            // and using TextEncoder (UTF-8) corrupts binary data like QR code headers.
            const encodedData = new Uint8Array(data.length);
            for (let i = 0; i < data.length; i++) {
                encodedData[i] = data.charCodeAt(i);
            }
            // --- END OF FIX ---

            // Chunk the byte array to prevent buffer overflow on mobile devices.
            const CHUNK_SIZE = 128; // Reduced chunk size for more reliability with complex binary data.
            const DELAY = 80;    // Increased delay to give the printer more time to process.
            ...
            for (let i = 0; i < encodedData.length; i += CHUNK_SIZE) {
                ...
                // A small delay is crucial for printers with small buffers.
