(function (global, factory) {
  ...
  class BLEPrinter {
      ...
      for (const svc of PRINTER_SERVICE_CANDIDATES) {
        try {
          ...
          if (writable) {
            ...
            this._preferWriteWithoutResponse = (!writable.properties.write && writable.properties.writeWithoutResponse);
            return;
          }
        } catch (_) { /* skip */ }
      }
    ...
    async write(data) {
      ...
      let CHUNK_SIZE = withResponse ? 180 : 18;     // <=— ubah ke 18
      let DELAY = withResponse ? 0 : 35;           // <=— ubah ke 35ms
      let DRAIN_EVERY_BYTES = withResponse ? 4096 : 2048;
      let DRAIN_DELAY = withResponse ? 50 : 300;   // <=— ubah ke 300ms
      ...
      if (!withResponse && isAndroid) {
        ...
        DRAIN_DELAY = Math.max(DRAIN_DELAY, 300);
      }
      ...
      for (let i = 0; i < data.length; i += CHUNK_SIZE) {
        ...
        while (true) {
          try {
            ...
            if (attempt <= 3) {
              await sleep(120);
              CHUNK_SIZE = Math.max(10, CHUNK_SIZE - 2);
              DELAY = DELAY + 10;
  ...
  class Printer {
    ...
    async printImage(imageUrl, options = {}) {
      ...
      const { align = 'center', maxWidth = defaultMaxWidth, threshold = 128, stripeHeight = 64 } = options;
